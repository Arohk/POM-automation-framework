...
public class BasePage {
	 ...
	 * animations)
	 * 
	 * @param elementToBeClickable
	 *            - element, for which the method waits to be available.
	 * @param elementToBeClicked
	 *            - element, which is actually clicked after.
	 ...
	 *            - Element to be clicked.
	 * @param secondsToWait
	 *            - Time available to try and click the element.
	 * @param conditionsNotMetText
	 *            - Text message returned to console if the click is not successful.
	 */
	 ...
	 *            - Element to be clicked.
	 * @param secondsToWait
	 *            - Time available for the click.
	 * @param threadSleepMilliseconds
	 *            - Mandatory timeout before and after the click.
	 */
...
//		executor.executeScript("arguments[0].setAttribute('style', 'border: 9px dashed #ff9999;');", element);
	 ...
	 * 
	 * @param element
	 *            - Element for text to be sent to.
	 * @param text
	 *            - Text to be sent to the element.
	 */
	 ...
	 * 
	 * @param element
	 *            - Element that needs to be cleared of text
	 */
	 ...
	 * 
	 * @param element
	 *            - Element, from which the text will be read.
	 ...
	 * @param pxDimensionWidth
	 *            - Width in pixels
	 * @param pxDimensionHeight
	 *            - Height in pixels
	 */
	...
	/**
	 * Switches to the passed iFrame of the game
	 * 
	 * @param frame
	 *            - passed frame
	 * @throws InterruptedException
	...
	/**
	 * Asserts that an element contains the String of expectedText that is entered
	 * with the actual one found from "BasePage.java"->read text method.
	 * 
	 * @param element
	 *            - Element, which text is to be returned by the read text method
	 *            and asserted with the expected text.
	 * @param expectedText
	 *            - Expected text that is to be found in the application.
	 */
	...
	/**
	 * Alternates the test result to PASSED if the next mission is not available, or
	 * continues with the test if the next mission is available due to content
	 * dripping
	 * 
	 * @param element
	 *            - Next available mission start/replay/continue button
	 * @param secondsToWait
	 *            - Seconds to wait for the start/replay/continue button to be
	 *            available
	 */
	public void isElementExistingAlternateResult(WebElement element, int secondsToWait) {
		boolean isElementFound = true;
		try {
			waitX = new WebDriverWait(driver, secondsToWait);
			waitX.until(ExpectedConditions.elementToBeClickable(element));
		} catch (Exception e) {
			isElementFound = false;
		}

		if (isElementFound == true) {
			System.out.println("Next mission available, test is continued...");
		} else {
			Reporter.getCurrentTestResult().setStatus(ITestResult.SUCCESS);
			System.out.println("test stopped and passed at " + element);
		}
	 ...
	 * 
	 * @param element
	 *            - element to check if it's being displayed
